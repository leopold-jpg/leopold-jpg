...
const svg = ({ cmd, meta }) => {
  ...
  <line x1="${ruleX}" y1="23" x2="${W - 16}" y2="23" stroke="${T.rule}" stroke-width="1.5"/>
  <text x="${W - 22}" y="27" text-anchor="end" font-family="${T.mono}" font-size="12" fill="${T.muted}">${meta}</text>
  <circle cx="${W - 6}" cy="23" r="3" fill="${T.accent}"><animate attributeName="opacity" values="1;0.35;1" dur="2.4s" repeatCount="indefinite"/></circle>
</svg>
`;
};
